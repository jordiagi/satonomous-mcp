...
var contractRoleSchema = z.enum(["buyer", "seller", "observer"]);
var contractActionSchema = z.enum([
  "fund_contract",
  "wait_for_funding",
  "submit_delivery",
  "wait_for_delivery",
...
var tokenProviderTypeSchema = z.enum(["local", "hosted", "byok", "brokered", "aggregator", "unknown"]);
var tokenProviderDisclosureSchema = z.enum(["exact", "class", "undisclosed"]);
var tokenAuthorizationBasisSchema = z.enum([
  "own_infrastructure",
  "authorized_resale",
  "aggregator_terms",
  "unknown",
...
var tokenPricingUnitSchema = z.enum(["per_1k_tokens", "per_1m_tokens"]);
var tokenMeteringMethodSchema = z.enum(["seller_signed_usage", "gateway_verified", "buyer_acknowledged"]);
var tokenCounterSchema = z.enum(["provider", "gateway", "tiktoken", "custom"]);
var tokenRetentionSchema = z.enum(["none", "hash_only", "redacted", "full", "custom"]);
var meteredEscrowStatusSchema = z.enum(["accepted", "funded", "active", "completed", "refunded", "disputed", "expired"]);
...
function formatSellerReputation(rep) {
...
function formatTenantReputation(rep) {
  return [
    ...
    rep.seller.summary.median_delivery_minutes !== null ? `  Median delivery: ${Math.round(rep.seller.summary.median_delivery_minutes)} min` : null,
    "",
...
function formatContractReceipt(receipt) {
  const verification = verifyContractReceipt(receipt);
  return [
    "\u{1F9FE} ContractReceipt v0",
...
function formatServiceCard(card) {
  const verification = verifyServiceCard(card);
  const reputation = card.seller.reputation ? `${card.seller.reputation.score}/100 ${card.seller.reputation.level}, ${formatNumber(card.seller.reputation.settled_contracts)} settled, ${formatPercent(card.seller.reputation.dispute_rate)} disputes` : "unavailable";
  return [
    "\u{1FAAA} ServiceCard v0",
...
function formatServiceCardList(cards) {
  return [
...
function formatTokenServiceCard(card) {
  const verification = verifyTokenServiceCard(card);
  const modelIds = card.inference.models.map((model) => model.id).join(", ");
  const provider = card.inference.provider.name ? `${card.inference.provider.name} (${card.inference.provider.type})` : card.inference.provider.type;
  const reputation = card.seller.reputation ? `${card.seller.reputation.score}/100 ${card.seller.reputation.level}, ${formatNumber(card.seller.reputation.settled_contracts)} settled` : "unavailable";
  return [
    "TokenServiceCard v0",
    `  Card ID: ${card.card_id}`,
    `  Body Hash: ${card.body_hash}`,
...
function formatMeteredEscrowContract(contract) {
...
function formatWalletPolicy(policy) {
  const verification = verifyWalletPolicy(policy);
  return [
    "WalletPolicy v0",
    `  Policy ID: ${policy.policy_id}`,
    `  Body Hash: ${policy.body_hash}`,
    policy.limits.max_contract_price_sats !== void 0 ? `  Max price: ${formatNumber(policy.limits.max_contract_price_sats)} sats` : null,
    policy.limits.max_contract_total_sats !== void 0 ? `  Max total: ${formatNumber(policy.limits.max_contract_total_sats)} sats` : null,
    policy.limits.daily_spend_limit_sats !== void 0 ? `  Daily limit: ${formatNumber(policy.limits.daily_spend_limit_sats)} sats` : null,
    policy.approvals.ask_human_above_sats !== void 0 ? `  Ask human above: ${formatNumber(policy.approvals.ask_human_above_sats)} sats` : null,
    `  Verification: ${verification.valid ? "valid" : verification.codes.join(", ")}`,
    "",
    "Raw JSON:",
...
function formatContractAction(action) {
  return [
...
function formatContractActionList(actions) {
  return [
...
function parseWalletPolicy(policyJson) {
...
async function createServer(config) {
  ...
  async function gatewayRequest(path, auth = true) {
  ...
  async function listGatewayOffers(filters, mine) {
  ...
  async function getGatewayReputation(tenantId) {
    const id = tenantId ?? (await gatewayRequest("/api/v1/tenants/me")).tenant_id;
    return gatewayRequest(`/api/v1/reputation/${encodeURIComponent(id)}`);
  }
  ...
  server.tool(
    ...
    "Register a new agent on the L402 Gateway to start trading services and escrow contracts.",
    {
      ...
      wallet_type: z.enum(["custodial", "external"]).optional().default("custodial").describe("Type of wallet"),
  ...
  server.tool("l402_balance", "Check your current balance on the L402 Gateway.", async () => {
    try {
      ...
      const text = `\u{1F4B0} Balance: ${formatNumber(info.balance_sats)} sats`;
      return { content: [{ type: "text", text }] };
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      return { content: [{ type: "text", text: `\u274C Error: ${msg}` }], isError: true };
  ...
  server.tool(
    "l402_deposit",
    "Create a Lightning invoice to deposit sats into your L402 account. IMPORTANT: AI agents cannot pay Lightning invoices \u2014 you MUST forward the invoice to a human for payment. Show them the invoice string and amount, then use l402_check_deposit to poll until paid.",
    {
    ...
    async ({ amount_sats, reason }) => {
      try {
        ...
        const text = [
          ...
          "",
          `\u{1F4F1} Tap to pay: lightning:${invoice.invoice}`,
          "",
          ...
          "",
          "\u{1F449} You (the AI agent) cannot pay this invoice.",
  ...
  server.tool(
    ...
    async ({ paymentHash }) => {
      try {
        ...
        const text = [
          `\u{1F4CB} Deposit Status: ${status.status}`,
  ...
  server.tool(
    ...
    "Publish a service offer for other agents to accept and purchase.",
    {
      ...
      sla_minutes: z.number().int().positive().optional().default(30).describe("Service level agreement - minutes until delivery required"),
      dispute_window_minutes: z.number().int().positive().optional().default(1440).describe("Dispute window - minutes buyer has to dispute after delivery")
    },
      ...
      try {
        const card = createTokenServiceCard({
          seller: {
            ...
            payout: args.lightning_address ? { lightning_address: args.lightning_address } : void 0,
            reputation: null,
            trust: {
          ...
          inference: {
            ...
            provider: {
              ...
              attestation: args.attestation ?? "Seller attests it is authorized to provide this inference service and is not sharing raw provider credentials."
            }
          },
          pricing: {
            currency: "sats",
          ...
          accept: {
            accept_url: args.accept_url ?? `satonomous://token-services/${encodeURIComponent(args.model_id)}/accept`,
            contract_template_ref: args.contract_template_ref ?? `satonomous:token-service:${args.model_id}`
          },
          links: {
  ...
  server.tool(
    ...
    async (args) => {
      try {
        const contract = JSON.parse(args.contract_json);
  ...
  server.tool(
    ...
    async ({ contract_json, status, closed_at }) => {
      try {
        ...
        const contract = JSON.parse(contract_json);
        const closed = closeMeteredEscrowContract(contract, status, closed_at);
      ...
      try {
        ...
        if (args.contractId) {
          ...
          request = {
            ...
            service_type: typeof terms.service_type === "string" ? terms.service_type : void 0
          };
          if (context.seller_reputation_score === void 0) {
            try {
              context = {
                ...context,
  ...
  server.tool(
    ...
    "List your contracts.",
    {
      role: z.enum(["buyer", "seller"]).optional().describe("Filter by your role"),
    ...
    async ({ role, status }) => {
      try {
        ...
        const text = [
          `\u{1F4CB} Contracts (${contracts.length} total):`,
          ...contracts.map(
            (c) => `  ${c.id}: ${c.status} \u2014 ${formatNumber(c.price_sats)} sats`
          )
        ].join("\n");
        return { content: [{ type: "text", text }] };
      } catch (err) {
  ...
  server.tool(
    ...
    "Poll a contract until it reaches a target next action or status.",
    {
      ...
      timeout_ms: z.number().int().positive().max(6e5).optional().default(6e4).describe("Maximum wait time"),
      poll_interval_ms: z.number().int().positive().max(6e4).optional().default(5e3).describe("Polling interval"),
      now: z.string().optional().describe("ISO timestamp for overdue calculation. Defaults to current time.")
    },
  ...
  No entries`
        ...
        const text = [
          `\u{1F4CA} Ledger \u2014 Balance: ${formatNumber(balance_sats)} sats`,
          ...entries.map(
            (e) => `  ${e.type === "credit" ? "\u2795" : "\u2796"} ${formatNumber(e.amount_sats)} \u2014 ${e.source} @ ${e.created_at}`
          )
        ].join("\n");
        return { content: [{ type: "text", text }] };
      } catch (err) {
...
// src/config.ts
var DEFAULT_API_URL = "https://l402gw.nosaltres2.info";
...
async function run() {
  ...
  const args = parseCliArgs();
  const options = {
    apiKey: args["api-key"] || void 0,
    apiUrl: args["api-url"] || void 0
  };
